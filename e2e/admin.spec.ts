...
test('platform admin creates a tenant and host, controls access, and preserves records when deleting the tenant', async ({ page, browser, baseURL, viewport, isMobile, hasTouch, browserErrors }, testInfo) => {
 ...
 for (const [label, value] of [['Tenant name', name], ['Portal slug', slug], ['Business time zone', 'Europe/Berlin'], ['Brand colour', '#FF0000'], ['Contact email', 'contact@example.test']]) {
 ...
 try {
  ...
  await host.getByRole('button', { name: 'Create listing', exact: true }).click(); await expect(host.getByRole('heading', { name: 'Your inventory', exact: true })).toBeVisible(); await host.getByRole('status').getByRole('link', { name: `Edit ${title}`, exact: true }).click(); await expect(host.getByRole('heading', { name: 'Edit listing', exact: true })).toBeVisible(); const listingId = new URL(host.url()).pathname.split('/').at(-1)!;
  await page.getByRole('link', { name: 'Back to tenant', exact: true }).click();
  ...
  try { await visitor.goto(`/${slug}`); await expect(visitor.getByRole('link', { name, exact: true })).toHaveCSS('color', 'rgb(255, 0, 0)'); await expect(visitor).toHaveTitle(`Explore stays · ${name}`); const primary = visitor.locator('.button-primary').first(); await expect(primary).toHaveCSS('background-color', 'rgb(255, 0, 0)'); await primary.hover(); await expect(primary).toHaveCSS('background-color', 'rgb(255, 0, 0)'); await expect(primary).toHaveCSS('color', 'rgb(0, 0, 0)'); await expect(visitor.getByRole('link', { name: title, exact: true })).toBeVisible(); await visitor.screenshot({ path: testInfo.outputPath('new-tenant-portal.png'), fullPage: true }); await visitor.goto('/citystays'); await expect(visitor.getByRole('link', { name: title, exact: true })).toHaveCount(0); expect((await visitor.request.get(`/api/v1/t/citystays/listings/${listingId}`)).status()).toBe(404); } finally { await visitorContext.close(); }
  await host.getByRole('link', { name: 'View public listing', exact: true }).click(); await expect(host.getByRole('heading', { name: title, exact: true })).toBeVisible(); await expect(host.getByText('Coordinates', { exact: true })).toHaveCount(0); await host.getByRole('button', { name: `Save ${title}`, exact: true }).click(); await expect(host.getByRole('button', { name: `Remove ${title} from saved listings`, exact: true })).toHaveCSS('color', 'rgb(23, 61, 50)');
  ...
  expect((await host.request.get(`/api/v1/t/${slug}/auth/me`)).status()).toBe(401); await host.goto(`/${slug}/host/listings`); await expect(host).toHaveURL(new RegExp(`/${slug}/login`));
  await row.getByRole('button', { name: 'Re-enable', exact: true }).click(); await page.getByRole('button', { name: 'Confirm re-enable', exact: true }).click(); await expect(row.getByRole('button', { name: 'Disable', exact: true })).toBeVisible(); expect((await host.request.get(`/api/v1/t/${slug}/auth/me`)).status()).toBe(401);
  await login(host, slug, hostEmail, changed); await expect(host).toHaveURL(new RegExp(`/${slug}/host/listings$`));
  await page.screenshot({ path: testInfo.outputPath('admin-accounts.png'), fullPage: true }); expect(await page.evaluate(() => document.documentElement.scrollWidth <= innerWidth)).toBe(true);
...
test('admin explicitly promotes a client with saved listings and assists another client with a password reset', async ({ page, browser, baseURL, viewport, isMobile, hasTouch, browserErrors }, testInfo) => {
 ...
 try {
  ...
  await login(client, 'greenstate', email, temp); await expect(client).toHaveURL(/\/greenstate\/password/); await client.goto('/greenstate/host/listings'); await expect(client).toHaveURL(/\/greenstate\/password/); await changePassword(client, temp, `Promoted password ${randomUUID()}!`, 'greenstate'); await client.getByRole('link', { name: 'Host workspace', exact: true }).click(); await expect(client.getByRole('heading', { name: 'Your inventory', exact: true })).toBeVisible(); await client.getByRole('navigation', { name: 'Main navigation' }).getByRole('link', { name: 'Saved listings', exact: true }).click(); await expect(client.getByRole('link', { name: listing.title, exact: true })).toBeVisible();
  await client.goto('/greenstate/account'); await client.getByRole('button', { name: 'Sign out', exact: true }).click(); await expect(client).toHaveURL(/\/greenstate\/login(?:\?|$)/); const resetEmail = `reset-client-${randomUUID()}@example.test`; await register(client, resetEmail, original); await client.goto(`/greenstate/listings/${listing.id}`); await client.getByRole('button', { name: `Save ${listing.title}`, exact: true }).click(); await expect(client.getByRole('button', { name: `Remove ${listing.title} from saved listings`, exact: true })).toBeVisible();
  row = await searchAccount(page, resetEmail); await row.getByRole('button', { name: 'Reset password', exact: true }).click(); await page.getByLabel('Temporary password', { exact: true }).fill(temp); await page.getByLabel(/I verified.*identity/).check(); await page.getByRole('button', { name: 'Confirm password reset', exact: true }).click(); await expect(page.getByText(/Password reset\. All sessions revoked/)).toBeVisible(); expect((await client.request.get('/api/v1/t/greenstate/auth/me')).status()).toBe(401);
  await login(client, 'greenstate', resetEmail, temp); await changePassword(client, temp, `Recovered client ${randomUUID()}!`, 'greenstate', '/greenstate'); await client.getByRole('navigation', { name: 'Main navigation' }).getByRole('link', { name: 'Saved listings', exact: true }).click(); await expect(client.getByRole('link', { name: listing.title, exact: true })).toBeVisible(); await expect(client.getByRole('link', { name: 'Host workspace', exact: true })).not.toBeVisible();
  await page.screenshot({ path: testInfo.outputPath('admin-lifecycle.png'), fullPage: true }); expect(await page.evaluate(() => document.documentElement.scrollWidth <= innerWidth)).toBe(true);
 } finally { await context.close(); }
});
