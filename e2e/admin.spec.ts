...
test('platform admin creates a tenant and host, controls access, and preserves records when deleting the tenant', async ({ page, browser, baseURL, viewport, isMobile, hasTouch, browserErrors }, testInfo) => {
 ...
 for (const [label, value] of [['Tenant name', name], ['Portal slug', slug], ['Business time zone', 'Europe/Berlin'], ['Brand colour', '#ffffff'], ['Contact email', 'contact@example.test']]) {
 ...
 try {
  ...
  await login(host, slug, hostEmail, changed); await expect(host).toHaveURL(new RegExp(`/${slug}/account$`));
...
test('admin explicitly promotes a client with saved listings and assists another client with a password reset', async ({ page, browser, baseURL, viewport, isMobile, hasTouch, browserErrors }, testInfo) => {
 ...
 try {
  ...
  await login(client, 'greenstate', resetEmail, temp); await changePassword(client, temp, `Recovered client ${randomUUID()}!`, 'greenstate'); await client.getByRole('navigation', { name: 'Main navigation' }).getByRole('link', { name: 'Saved listings', exact: true }).click(); await expect(client.getByRole('link', { name: listing.title, exact: true })).toBeVisible(); await expect(client.getByRole('link', { name: 'Host workspace', exact: true })).not.toBeVisible();
