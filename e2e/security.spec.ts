...
test('host tabs revoke access on admin disable and reset while retaining data and escaping listing text', async ({ page, context, browser, baseURL, browserErrors }) => {
 ...
 try {
  ...
  await activateTab(page); await login(page, 'greenstate', host.email, changed); await expect(page).toHaveURL(/\/greenstate\/host\/listings$/); await second.goto('/greenstate/saved'); await expect(second.getByRole('link', { name: title, exact: true })).toBeVisible();
