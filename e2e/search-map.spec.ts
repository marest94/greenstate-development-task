...
test('map pins and cards stay synchronized through selection, pagination and filter removal', async ({ page, request }, testInfo) => {
  ...
  const first = firstPage.items[0]!; const second = firstPage.items[1]!;
  const sharedPin = first.latitude === second.latitude && first.longitude === second.longitude;
  ...
  const pin = panel.getByRole('button', { name: sharedPin ? 'Show 2 stays at this location' : `Show ${first.title} on map`, exact: true });
  await pin.click();
  ...
  await expect(panel.getByRole('button', { name: sharedPin ? 'Show 2 stays at this location' : `Show ${second.title} on map`, exact: true })).toHaveAttribute('aria-pressed', 'true');
  await expect(panel.getByRole('link', { name: second.title, exact: true })).toBeVisible();
