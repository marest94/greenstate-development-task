...
export const HostListingsQuerySchema = PaginationSchema.extend({ status: z.enum(['active', 'archived', 'all']).default('active'), search: z.string().trim().min(1).max(200).optional(), city: z.string().trim().min(1).max(80).optional(), propertyType: fields.propertyType.optional(), sort: z.enum(['title', 'price-asc', 'price-desc']).default('title') });
