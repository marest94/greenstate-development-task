...
export function AuthNavigation() {
  ...
  if (auth.principal) return <>{auth.principal.permissions.includes('tenants:manage') && !auth.principal.mustChangePassword && <NavLink className="auth-nav-link" to="/admin/tenants">Administration</NavLink>}{auth.scope.realm === 'tenant' && !auth.principal.mustChangePassword && <NavLink className="auth-nav-link" to={`${auth.basePath}/saved`}>Saved listings</NavLink>}{auth.principal.permissions.includes('listings:manage') && !auth.principal.mustChangePassword && <NavLink className="auth-nav-link" to={`${auth.basePath}/host/listings`}>Host workspace</NavLink>}<NavLink className="auth-nav-link" to={`${auth.basePath}/account`}>Account</NavLink></>;
