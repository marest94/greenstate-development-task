...
export function LoginPage() {
  ...
  if (auth.principal) return <Navigate to={afterAuthentication(auth.principal, auth.basePath, requested)} replace />;
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    feedback.clear();
    try { const principal = await auth.login(parsed.data); clearPasswords(form); navigate(afterAuthentication(principal, auth.basePath, requested), { replace: true }); }
