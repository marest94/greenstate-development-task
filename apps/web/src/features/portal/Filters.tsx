...
export function Filters({ value, resetKey, cities, today, onSubmit, onClearDates, onReset }: Props) {
  ...
  return <form className="search-filters" aria-label="Find a stay" onSubmit={submit} noValidate>
    ...
    <div className="filter-grid">
      <label className="filter-city" htmlFor={`${id}-city`}>City<select id={`${id}-city`} value={city} onChange={event => setCity(event.target.value)}><option value="">All cities</option>{cityOptions.map(option => <option key={option}>{option}</option>)}</select></label>
      <label className="filter-guests" htmlFor={`${id}-guests`}>Guests<input id={`${id}-guests`} type="number" inputMode="numeric" min="1" max="12" step="1" placeholder="Any" value={guests} onChange={event => setGuests(event.target.value)} /></label>
      <DateRangeField value={dates} today={today ?? ''} onChange={changeDates} />
      <label className="filter-min" htmlFor={`${id}-min`}>Minimum price (€)<input id={`${id}-min`} type="text" inputMode="decimal" placeholder="No minimum" value={minPrice} onChange={event => setMinPrice(event.target.value)} /></label>
      <label className="filter-max" htmlFor={`${id}-max`}>Maximum price (€)<input id={`${id}-max`} type="text" inputMode="decimal" placeholder="No maximum" value={maxPrice} onChange={event => setMaxPrice(event.target.value)} /></label>
    </div>
