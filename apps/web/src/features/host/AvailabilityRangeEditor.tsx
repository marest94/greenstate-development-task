...
export function AvailabilityRangeEditor({ row, selected, today, busy, reason, message, problem, basePath, onReason, onDates, onSave, onClose }: {
  ...
  return <RangeShell onClose={onClose} busy={busy}>
    ...
    {inView && <><div className="range-preview"><strong>{days.length} {days.length === 1 ? 'night' : 'nights'} selected</strong><p>{free} available · {occupied} booked · {blocks} manually blocked</p></div>
      ...
      {canBlock && <><label className="host-field">Reason (optional)<textarea maxLength={500} value={reason} disabled={busy} onChange={e => onReason(e.target.value)} /></label><p>{free} {free === 1 ? 'night' : 'nights'} will be blocked</p>{blocks > 0 && <p>Existing blocks and their reasons will be kept.</p>}<button className="button-primary" disabled={busy} onClick={() => onSave('block')}>{busy ? 'Saving…' : `Block ${free} ${free === 1 ? 'night' : 'nights'}`}</button></>}
      {blocks > 0 && <><p>{blocks} manual {blocks === 1 ? 'block' : 'blocks'} can be removed. Bookings remain unchanged.</p><button className="button-secondary" disabled={busy} onClick={() => onSave('unblock')}>Remove {blocks} {blocks === 1 ? 'block' : 'blocks'}</button><ul className="range-blocks">{days.filter(d => d.block).map(d => <li key={d.date}>{formatDate(d.date)}: {d.block?.reason ?? 'No reason provided'}</li>)}</ul></>}
