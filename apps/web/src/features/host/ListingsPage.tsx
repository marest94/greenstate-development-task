...
export function ListingsPage() {
  ...
  function change(values: Record<string, string>) { setParams({ status: parsed.success ? parsed.data.status : 'active', page: String(parsed.success ? parsed.data.page : 1), pageSize: String(parsed.success ? parsed.data.pageSize : 20), ...values }); }
  return <section aria-labelledby="inventory-heading">
    ...
    {query.data && <><p className="host-count" role="status">{query.data.total} {query.data.total === 1 ? 'listing' : 'listings'}</p>
      {query.data.items.length ? <div className="host-table-scroll"><table className="host-table"><caption className="host-sr-only">Tenant listing inventory</caption><thead><tr><th scope="col">Listing</th><th scope="col">Location</th><th scope="col">Capacity</th><th scope="col">Per night</th><th scope="col">Status</th></tr></thead><tbody>
        {query.data.items.map(item => <tr key={item.id}><th scope="row"><Link to={`${auth.basePath}/host/listings/${item.id}`}>{item.title}</Link></th><td>{item.city}, {item.country}</td><td>{item.maxGuests} guests</td><td>{formatMoney(item.pricePerNightCents)}</td><td><span className="host-badge">{item.archivedAt ? 'Archived' : 'Active'}</span></td></tr>)}
