...
export async function lockLiveTenant(tx: Prisma.TransactionClient, tenantId: string, mode: 'shared' | 'exclusive') {
  ...
  if (mode === 'shared') await tx.$executeRaw`SELECT pg_advisory_xact_lock_shared(7101, hashtext((${tenantId}::uuid)::text))`;
  else await tx.$executeRaw`SELECT pg_advisory_xact_lock(7101, hashtext((${tenantId}::uuid)::text))`;
